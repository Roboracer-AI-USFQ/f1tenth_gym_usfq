...
class CrossQSAC_Agent(Base_Agent):
    ...
    def rollout(self, episodes: int, train: bool) -> None:
        """
...
class CrossQTD3_Agent(Base_Agent):
    ...
    def rollout(self, max_steps: int, train: bool) -> None:
        ...
        for _ in range(max_steps):
            ...
            ):  # TODO: add behavior for truncated episodes
                action = self.select_action(
                    state, train
                )  # While rollouts the target actor is used
    ...
    def train(
        ...
        for global_step in range(total_steps):
            ...
            for train_step in range(self.training_steps_per_rollout):
                ...
                target_q = (
                    reward_tensor + (1 - terminated_tensor) * self.gamma * target_q
                )
