...
class Base_Agent:
    ...
    def _do_random_actions(self, initial_steps: int) -> None:
        ...
        for i in range(initial_steps):
            state, info = self.env.reset()  # Reset env and get initial state (gymnasium format)
            ...
            while not terminated and not truncated:  
                action = np.random.uniform(
                    low=np.array([underlying_env.params["sv_min"], underlying_env.params["v_min"]]),
                    high=[underlying_env.params["sv_max"], underlying_env.params["v_max"]],
                    size=(1, 2)
                ).astype(np.float32)

                next_state, reward, terminated, truncated, info = self.env.step(action)  # gymnasium format
